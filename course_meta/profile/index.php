...
// kick off by ensuring vw_course_metadata reflects the latest fields (create or replace view)
...
foreach ($categories as $category) {
    ...
    $table->head  = array(get_string('profilefield', 'admin'), get_string('description'), get_string('fieldtype', 'local_course_meta'), get_string('edit'));
    ...
    if ($fields = $DB->get_records('course_meta_info_field', array('categoryid' => $category->id), 'sortorder ASC')) {
        foreach ($fields as $field) {
            $table->data[] = array(format_string($field->name), format_string($field->description), format_string($field->datatype), profile_field_icons($field));
        }
    }
